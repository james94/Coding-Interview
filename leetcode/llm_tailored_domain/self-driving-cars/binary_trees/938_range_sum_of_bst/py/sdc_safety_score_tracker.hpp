...
private:
    ...
    int calculateSafetyScoreHelper(std::shared_ptr<DecisionNode> root, int min_threshold, int max_threshold) {
        if (root == nullptr) {
            return 0;
        }
...
public:
    int calculateSafetyScore(std::shared_ptr<DecisionNode> root, int min_threshold, int max_threshold) {
